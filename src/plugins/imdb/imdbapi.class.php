...
class IMDB {
     ...
     * @return boolean
     */
    private function isHttpsButtflare() {
        $whitelist = array(
            '127.0.0.1',
            '::1'
        );
    ...
    public function getBasePath(){
        ...
        return (($this->isHttpsButtflare()) ? 'https' : 'http') . '://'.$dir;
    }
    ...
    public function getCastNameArray() {
        $cast_list = array();
        if(isset($this->data['cast_summary'])){
            foreach ($this->data['cast_summary'] as $cast) {
                $cast_list[] = $this->removeAccents($cast['name']['name']);
            }
        }
        return $cast_list;
    }

    //Can set the max amount of cast name to return
    public function getCastNameString($max = 0) {
        return trim(implode(" | ", $this->getCastNameArray($max)), " | ");
    }

    //Can set the max amount of cast name to return
    public function getCastNameStringCommas($max = 0) {
        return trim(implode(", ", $this->getCastNameArray($max)), ", ");
    }
    ...
    //Can set the max amount of cast name to return
    public function getDirectorNameString($max = 0) {
        return trim(implode(" | ", $this->getDirectorNameArray($max)), " | ");
    }

    //Can set the max amount of cast name to return
    public function getDirectorNameStringCommas($max = 0) {
        return trim(implode(", ", $this->getDirectorNameArray($max)), ", ");
    }
    ...
    //Can set the max amount of generes to return
    public function getGenreStringCommas($max = 0) {
        return trim(implode(", ", $this->getGenreArray($max)), ", ");
    }
